...
ask_ques = "**» يرجـى اختيـار أحـد الجلسات التاليـة :**"
buttons_ques = [
    [
    ...
    [
        InlineKeyboardButton("بايروجـرام bot", callback_data="pyrogram_bot"),
        InlineKeyboardButton("تليثـون bot", callback_data="telethon_bot"),
    ],
]

gen_button = [
    [
        InlineKeyboardButton(text="- استخـراج الجلسـة .", callback_data="generate")
    ]
]
...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    if is_bot:
        ty += "بوت"
    await msg.reply(f"**» استخـراج الجلسـة **{ty}** بواسطـة 𝗣𝗼𝘄𝗲𝗿𝘁𝗵𝗼𝗻 جـارٍ..**.")
    user_id = msg.chat.id
    ...
    text = f"**هذا هو {ty} كـود جلسـة** \n\n`{string_session}` \n\n**مستخـرج مـن :** @FJDbot\n🍒 **ملاحظـة :** لا تشارك الكود لأحـد لأنـه يستطيع اختراق حسابك من خلالـه قنـاة السورس @Power_Thon ."
    try:
        if not is_bot:
